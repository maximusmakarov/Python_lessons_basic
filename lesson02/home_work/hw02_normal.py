...
print('все положительные числа списка')
print(numbers)
for number in numbers:
    if math.sqrt(number) != math.ceil(math.sqrt(number)):
        numbers.remove(number)
    else:
        final_numbers.append(int(math.sqrt(number)))
print('результаты извлечения корня не имеют десятичной части')
print(final_numbers)
...
while True:
    try:
        ...
        print(days[date_input[0]] + ' ' + months[date_input[1]] + ' ' + date_input[2] + ' ' + 'года')
    except KeyError:
...
print('количество элементов списка: ' + n)
print(mas)
